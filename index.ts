...
connect().then(async () => {
  // channel.deleteQueue("email");

  await channel.consume("email", async (bodyData: any) => {
    try {
        ...
        <body>
          <header className="clearfix">
            ...
            <div id="company" className="clearfix">
              <div><span>INVOICE DATE : </span>${data?.createdAt}</div>
              <div><span>INVOICE REFERENCE : </span></div>
            </div>
            <div id="project">
              <div><span>SUPPLIER</span>${data?.user}</div>
              <div><span>EMAIL</span>${data?.userEmail || "NA"}</div>
            </div>
          </header>
          <main>
            <table>
              ...
              <tbody>
               ...
               ${data?.products?.map((product: any) => {
                ...
                <tr>
                  ...
                  <td className="total">${data?.totalPrice}</td>
                </tr>
...
app.listen(process.env.PORT || 3005, () => {
  console.log("Email service is listening on port 3005");
});
